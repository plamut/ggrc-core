...
(function (can) {
  ...
  function makeDateSerializer(type, key) {
    ...
    return function (d) {
      ...
      retstr = moment((d / 1000).toString(), 'X');
      if (type !== 'date') {
        retstr = retstr.utc();
      }
  ...
  can.Model('can.Model.Cacheable', {
    ...
    setup: function (construct, name, statics, prototypes) {
      ...
      if (this.root_collection) {
        this.model_plural = statics.model_plural ||
          this.root_collection.replace(/(?:^|_)([a-z])/g, function (s, l) {
            return l.toUpperCase();
          });
        this.title_plural = statics.title_plural ||
          this.root_collection.replace(/(^|_)([a-z])/g, function (s, u, l) {
            return (u ? ' ' : '') + l.toUpperCase();
          });
        this.table_plural = statics.table_plural || this.root_collection;
      }
      if (this.root_object) {
        this.model_singular = statics.model_singular ||
          this.root_object.replace(/(?:^|_)([a-z])/g, function (s, l) {
            return l.toUpperCase();
          });
        this.title_singular = statics.title_singular ||
          this.root_object.replace(/(^|_)([a-z])/g, function (s, u, l) {
            return (u ? ' ' : '') + l.toUpperCase();
          });
    ...
    model: function (params) {
      ...
      m = this.findInCacheById(params[this.id]) ||
        (params.provisional_id && can.getObject('provisional_cache',
    ...
    parse_deep_property_descriptor: function (deep_property_string) {
      return Object.freeze(_.map(deep_property_string.split('.'),
        function (part) {
          if (part === 'GET_ALL') {
            return part;
          }
      ...
      return this.get_list_loader('orphaned_objects').then(function (list) {
        ...
        function is_join(mapping) {
          ...
          if (mapping.mappings.length > 0) {
            for (; (child = mapping.mappings[i]); i++) {
          ...
          return mapping.instance &&
            mapping.instance instanceof can.Model.Join && mapping.instance;
        }
        can.each(list, function (mapping) {
          var inst = is_join(mapping);
          if (inst) {
            mappings.push(inst);
          } else {
        ...
        if (objects.length) {
          ...
          can.each(counts, function (count, i) {
            parts++;
            result.push(count.count + ' ' + (count.count === 1 ?
              count.model.title_singular : count.model.title_plural) + ',');
          });
        }
        if (mappings.length) {
          parts++;
      ...
      this.each(function (val, name) {
        ...
        if (that.constructor.attributes && that.constructor.attributes[name]) {
          ...
          serial[name] = can.map(val, function (v) {
            ...
            return v.serialize ? v.serialize() : v;
          });
          ...
          if (that[name] && that[name].isComputed) {
            ...
            serial[name] = that[name] && that[name].serialize ?
              that[name].serialize() : that._super(name);
          }
        }
      });
      return serial;
    ...
    get_permalink: function () {
      ...
      $.when(this.refresh_all.apply(this,
        constructor.permalink_options.base.split(':'))).then(function (base) {
          return dfd.resolve(_.template(
            constructor.permalink_options.url)({base: base, instance: this}));
        }.bind(this));
      return dfd.promise();
    },
    ...
    */
    ...
    _save: function () {
      ...
      var xhr;
      var dfd = this._dfd;
      var pre_save_notifier = new PersistentNotifier({
      ...
      pre_save_notifier.on_empty(function () {
        ...
        .then(function (result) {
          if (isNew) {
            if (that.after_create) {
              that.after_create();
            }
          } else if (that.after_update) {
            that.after_update();
          }
          if (that.after_save) {
            that.after_save();
          ...
          if (that.save_error) {
            that.save_error(xhr.responseText);
          }
          return $.Deferred().reject(xhr, status, message);
        })
    ...
    get_deep_property: function (property_descriptor) {
      ...
      function map_deep_prop(index) {
        return function (element) {
          return element.get_deep_property(
            property_descriptor.slice(index + 1));
        };
      }
  ...
  can.Observe('can.Stub', {
    ...
    init: function () {
      ...
      this._instance().bind('destroyed', function (ev) {
        ...
        can.trigger(that, 'change',
          ['stub_destroyed', 'stub_destroyed', that, null]);
