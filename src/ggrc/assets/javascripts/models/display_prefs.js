...
(function (can, $) {
  ...
  can.Model.LocalStorage('CMS.Models.DisplayPrefs', {
    ...
    setCollapsed: function (page_id, widget_id, is_collapsed) {
      this.makeObject(page_id === null ? page_id : path, COLLAPSE)
        .attr(widget_id, is_collapsed);
      if (this.autoupdate) {
        this.save();
      }
      return this;
    ...
    getCollapsed: function (page_id, widget_id) {
      var collapsed = this.getObject(
        page_id === null ? page_id : path, COLLAPSE);
      if (!collapsed) {
        collapsed = this.makeObject(page_id === null ? page_id : path, COLLAPSE)
          .attr(this.makeObject(COLLAPSE, page_id).serialize());
      }
      return widget_id ? collapsed.attr(widget_id) : collapsed;
    },
    setTopNavHidden: function (page_id, is_hidden) {
      this.makeObject(page_id === null ? page_id : path, TOP_NAV)
        .attr('is_hidden', !!is_hidden);
      if (this.autoupdate) {
        this.save();
      }
      return this;
    ...
    setModalState: function (model_name, display_state) {
      var path = null;
      var modal_state = this.getObject(path, MODAL_STATE);
      var obj = {};
    ...
    getLHNavSize: function (page_id, widget_id) {
      ...
      if (!size) {
        size = this.makeObject(page_id === null ? page_id : path, LHN_SIZE)
        .attr(this.makeObject(LHN_SIZE, page_id).serialize());
      }
      return widget_id ? size.attr(widget_id) : size;
    },
    setGlobal: function (widget_id, attrs) {
      var global = this.getObject(null, GLOBAL) &&
        this.getObject(null, GLOBAL).attr(widget_id);

      if (!global) {
        global = this.makeObject(null, GLOBAL)
          .attr(widget_id, new can.Observe(attrs));
      } else {
        global.attr(attrs);
      }
      if (this.autoupdate) {
    ...
    getSorts: function (page_id, column_id) {
      ...
      if (!sorts) {
        sorts = this.makeObject(path, SORTS)
          .attr(this.makeObject(SORTS, page_id).serialize());
        if (this.autoupdate) {
          this.save();
        }
      }
    ...
    setSorts: function (page_id, widget_id, sorts) {
      ...
      page_sorts = this.makeObject(path, SORTS);
      page_sorts.attr(
        widget_id ? widget_id : sorts, widget_id ? sorts : undefined);
      if (this.autoupdate) {
        this.save();
      }
      return this;
    ...
    getWidgetHeights: function (page_id) {
      ...
      if (!heights) {
        heights = this.makeObject(path, HEIGHTS)
          .attr(this.makeObject(HEIGHTS, page_id).serialize());
        if (this.autoupdate) {
          this.save();
        }
      }
    ...
    getColumnWidths: function (page_id, content_id) {
      ...
      if (!widths) {
        widths = this.makeObject(path, COLUMNS)
          .attr(this.makeObject(COLUMNS, page_id).serialize());
        if (this.autoupdate) {
          this.save();
        }
      }
  ...
  // reset function currently resets all layout for a page type (first element in URL path)
    ...
    setPageAsDefault: function (page_id) {
      ...
      can.each([COLLAPSE, LHN_SIZE, OBJ_SIZE, SORTS, HEIGHTS, COLUMNS],
        function (key) {
          that.makeObject(key).attr(page_id,
            new can.Observe(that.makeObject(path, key).serialize()));
        });
      this.save();
      return this;
    },
    ...
    setPbcListPrefs: function (pbc_id, prefs) {
      this.makeObject(PBC_LISTS).attr(pbc_id,
          prefs instanceof can.Observe ? prefs : new can.Observe(prefs));
      if (this.autoupdate) {
        this.save();
      }
    },
    ...
    setLHNState: function (new_prefs, val) {
      ...
      can.each(['open_category', 'panel_scroll', 'category_scroll',
