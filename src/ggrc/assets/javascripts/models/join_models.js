...
(function (can, $) {
  can.Model.Cacheable('can.Model.Join', {
    ...
    init_join_object_with_type: function (attr) {
      ...
      object_id = this[attr + '_id'] || (this[attr] || {}).id;
      object_type = this[attr + '_type'] || (this[attr] || {}).type;

      if (object_id && object_type && typeof object_type === 'string') {
        this.attr(attr, CMS.Models.get_instance(object_type,
          object_id, this[attr]) || this[attr]);
      } else if (object_id) {
    ...
    init_join_object: function (attr, model_name) {
      ...
      if (object_id) {
        this.attr(attr, CMS.Models.get_instance(
              model_name, object_id, this[attr]
            ).stub() || this[attr]);
      }
    },
  ...
  can.Model.Join('CMS.Models.Relationship', {
    ...
    reinit: function () {
      this.attr('source', CMS.Models.get_instance(
        this.source_type ||
          (this.source &&
            (this.source.constructor && this.source.constructor.shortName ||
            (!this.source.selfLink && this.source.type))), this.source_id ||
            (this.source && this.source.id), this.source) || this.source);
  ...
  can.Model.Join('CMS.Models.UserRole', {
    ...
    save: function () {
      ...
      if (!this.role && this.role_name) {
        ...
        return CMS.Models.Role.findAll({name__in: this.role_name})
          .then(function (roles) {
            if (roles.length < 1) {
