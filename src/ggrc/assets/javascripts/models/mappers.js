    ...
    insert_results: function (binding, results) {
      ...
      can.each(results, function (new_result) {
        ...
        if (!found_result && binding.pending_list) {
          found_result = self.find_result_by_instance(
            new_result, binding.pending_list);
        }
        if (!found_result) {
          found_result = self.find_result_by_instance(
            new_result, new_instance_results);
        }
        if (found_result) {
          ...
          found_result = self.make_result(
            new_result.instance, new_result.mappings, binding);
    ...
    init_listeners: function (binding) {
      ...
      binding.source_binding.list.bind('add', function (ev, results) {
        if (binding._refresh_stubs_deferred && binding._refresh_stubs_deferred.state() !== 'pending') {
          matching_results = can.map(can.makeArray(results), function (result) {
  ...
  GGRC.ListLoaders.BaseListLoader('GGRC.ListLoaders.CrossListLoader', {
    ...
    _refresh_stubs: function (binding) {
      ...
      return binding.source_binding.refresh_stubs().then(function (local_results) {
        ...
        can.each(local_results, function (local_result) {
          var remote_binding = self.insert_local_result(binding, local_result);
          var deferred = remote_binding.refresh_instance().then(function () {
  ...
  */
        ...
        binding.refresh_instances().done(function () {
          new RefreshQueue().enqueue(
            ...
            can.map(can.makeArray(results), function (result) {
              var new_result = self.make_result(
                result.instance, [result], binding);
              new_result.compute = can.compute(function () {
                return self.filter_fn(result);
              });
              new_result.compute.bind('change',
                $.proxy(self, 'process_result', binding, result, new_result));
              self.process_result(
                binding, result, new_result, new_result.compute());
            });
          });
        });
      });
        ...
        .then(function (results) {
          new RefreshQueue().enqueue(
            ...
            can.map(can.makeArray(results), function (result) {
              ...
              new_result.compute.bind('change',
                $.proxy(self, 'process_result', binding, result, new_result));
              self.process_result(binding,
                result, new_result, new_result.compute());
            });
          });
        });
    }
  ...
  GGRC.ListLoaders.BaseListLoader('GGRC.ListLoaders.IntersectingListLoader', {
    ...
    init_listeners: function (binding) {
      ...
      var self = this;
      var new_results;
      var lists = can.map(binding.source_bindings, function (source) {
        return [can.map(
          source.list,
          function (result) {
  ...
  GGRC.ListLoaders.BaseListLoader('GGRC.ListLoaders.ProxyListLoader', {
    ...
    init_listeners: function (binding) {
      ...
      var object_join_value = binding.instance[this.object_join_attr];
    ...
    is_valid_mapping: function (binding, mapping) {
      var model = CMS.Models[this.model_name];
      var object_model = binding.instance.constructor;
      var option_model = CMS.Models[this.option_model_name];
  ...
  GGRC.ListLoaders.BaseListLoader('GGRC.ListLoaders.DirectListLoader', {
    ...
    _refresh_stubs: function (binding) {
      ...
      return refresh_queue.trigger().then(function () {
        ...
        var mappings = binding.instance[object_join_attr] &&
          binding.instance[object_join_attr].reify();
  ...
  GGRC.ListLoaders.BaseListLoader('GGRC.ListLoaders.IndirectListLoader', {
    ...
    _refresh_stubs: function (binding) {
      var model = CMS.Models[this.model_name];
      var object_join_attr = ('indirect_' +
        (this.object_join_attr || model.table_plural));
      var mappings = binding.instance[object_join_attr] &&
        binding.instance[object_join_attr].reify();
      var params = {};
      var object_attr = this.object_attr + (this.object_attr !== 'context' &&
        model.attributes[this.object_attr].indexOf('stubs') > -1 ? '.id' : '_id');
      var self = this;

      if (this.object_attr === 'context') {
        params[object_attr] = binding.instance.context &&
          binding.instance.context.id;
      } else {
  ...
  GGRC.ListLoaders.BaseListLoader('GGRC.ListLoaders.SearchListLoader', {
    ...
    _refresh_stubs: function (binding) {
      var object_join_attr = ('search_' + (this.object_join_attr ||
