...
(function (can) {
  ...
  can.Model.Cacheable('CMS.Models.Audit', {
      ...
      this.validate(['_transient.audit_firm', 'audit_firm'],
        function (newVal, prop) {
          ...
          if (!audit_firm && transient_audit_firm) {
            if (_.isObject(transient_audit_firm) &&
                (audit_firm.reify().title !== transient_audit_firm.reify().title) ||
                (transient_audit_firm !== '' && transient_audit_firm !== null &&
                  audit_firm !== null && transient_audit_firm !== audit_firm.reify().title)) {
    ...
    get_filter_vals: function () {
      ...
      var keys = this.class.filter_keys.concat(['state', 'code', 'audit lead']);
  ...
  can.Model.Cacheable('CMS.Models.Request', {
    ...
    tree_view_options: {
      ...
      attr_list: [
        ...
        {attr_title: 'Status', attr_name: 'status'},
        {attr_title: 'Last Updated', attr_name: 'updated_at'},
        {attr_title: 'Request Date', attr_name: 'requested_on',
          attr_sort_field: 'report_start_date'},
        {attr_title: 'Due Date', attr_name: 'due_on',
    ...
    form_preload: function (new_object_form) {
      ...
      if (new_object_form && page_instance && page_instance.type === 'Audit') {
        if (!this.audit) {
          ...
          this.mark_for_addition('related_objects_as_destination',
            page_instance.program);
        }
      }
    }
  });
