...
function _display_tree_subpath(el, path, attempt_counter) {
  ...
  var rest = path.split('/'), type = rest.shift(), id = rest.shift(), selector = '[data-object-type=\'' + type + '\'][data-object-id=' + id + ']', $node, $next_node, node_controller, scroll_delay = 0;
...
can.Observe('can.Observe.TreeOptions', {
  defaults: {
    instance: undefined, parent: null, children_drawn: false
  }
}, {
  // init : function() {
...
can.Control('CMS.Controllers.TreeLoader', {
  ...
  prepare: function () {
    ...
    var that = this, tracker_stop = GGRC.Tracker.start(
    ...
    // TODO figure out why this happens and fix the root of the problem
      ...
      var that = this, refresh_queue = new RefreshQueue(), sort_function, original_function, sort_prop;
...
CMS.Controllers.TreeLoader('CMS.Controllers.TreeView', {
  //static properties
  defaults: {
    model: null, header_view: GGRC.mustache_path + '/base_objects/tree_header.mustache', show_view: null, show_header: false, footer_view: null, add_item_view: null, parent: null, list: null, single_object: false, find_params: {}, sort_property: null, sort_direction: null, sort_by: null, sort_function: null, sortable: true, filter: null, start_expanded: false //true
    , draw_children: true, find_function: null, options_property: 'tree_view_options', allow_reading: true, allow_mapping: true, allow_creating: true, child_options: [] //this is how we can make nested configs. if you want to use an existing
    ...
    CMS.Models.DisplayPrefs.getSingleton().then(function (display_prefs) {
      ...
      if ('parent_instance' in opts && 'status' in opts.parent_instance) {
        var setAllowMapping = function () {
          var is_accepted = opts.parent_instance.attr('status') === 'Accepted', admin = Permission.is_allowed('__GGRC_ADMIN__')
            ;
  ...
  }, 100, {leading: true}), _last_scroll_top: 0, _is_scrolling_up: false, '{scroll_element} scroll': function (el, ev) {
  ...
  , add_child_lists: function (list) {
      var that = this, current_list = can.makeArray(list), list_window = [], final_dfd, queue = [], op_id = this._add_child_lists_id = (this._add_child_lists_id || 0) + 1
      ;
    ...
    var that = this, $footer = this.element.children('.tree-item-add').first(), $items = $(), $existing = this.element.children('.cms_controllers_tree_view_node'), draw_items_dfds = [], sort_prop = this.options.sort_property, sort_function = this.options.sort_function, filter = this.options.filter
        ;
    ...
    that.element.children([
      '[data-object-id=' + instance.id + ']', '[data-object-type=' + instance.constructor.table_singular + ']'
    ].join('')
    ).remove();
    ev.stopPropagation();
...
can.Control('CMS.Controllers.TreeViewNode', {
  defaults: {
    model: null, parent: null, instance: null, options_property: 'tree_view_options', show_view: null, expanded: false, draw_children: true, child_options: []
  }
}, {
  setup: function (el, opts) {
    var that = this;
  ...
  , add_child_lists_to_child: function () {
      var that = this, original_child_options = this.options.child_options, new_child_options = [];
      ...
      can.each(original_child_options, function (data, i) {
        ...
        options.attr({
          'options_property': this.options.options_property, 'single_object': false, 'parent': this, 'parent_instance': this.options.instance
        });
  ...
  , add_child_list: function (item, data) {
    ...
    var old_el = this.element, $el, old_data, i, firstchild
      ;
    ...
    return this.trigger_expand();
      ...
      var that = this, $openclose = this.element.find('.openclose').first();
