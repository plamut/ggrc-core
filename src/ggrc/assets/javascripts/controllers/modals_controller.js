...
(function (can, $) {
  ...
  BUTTON_VIEW_DONE: GGRC.mustache_path + '/modals/done_buttons.mustache', BUTTON_VIEW_CLOSE: GGRC.mustache_path + '/modals/close_buttons.mustache'
  //  BUTTON_VIEW_SAVE
  , BUTTON_VIEW_SAVE_CANCEL: GGRC.mustache_path + '/modals/save_cancel_buttons.mustache', BUTTON_VIEW_SAVE_CANCEL_DELETE: GGRC.mustache_path + '/modals/save_cancel_delete_buttons.mustache', defaults: {
    preload_view: GGRC.mustache_path + '/dashboard/modal_preload.mustache', content_view: GGRC.mustache_path + '/help/help_modal_content.mustache', header_view: GGRC.mustache_path + '/modals/modal_header.mustache', custom_attributes_view: GGRC.mustache_path + '/custom_attributes/modal_content.mustache', button_view: null, model: null    // model class to use when finding or creating new
    , instance: null // model instance to use instead of finding/creating (e.g. for update)
    , new_object_form: false, mapping: false, find_params: {}, add_more: false, ui_array: [], reset_visible: false
  }, init: function () {
    ...
    .ggrc_controllers_modals(can.extend({
      new_object_form: false, button_view: GGRC.mustache_path + '/modals/confirm_buttons.mustache', modal_confirm: 'Confirm', modal_description: 'description', modal_title: 'Confirm', content_view: GGRC.mustache_path + '/modals/confirm.mustache'
    }, options))
  ...
  immediate_find_or_create: function (el, ev, data) {
    var that = this, prop = el.data('drop'), model = CMS.Models[el.data('lookup')], params = {context: that.options.instance.context && that.options.instance.context.serialize ? that.options.instance.context.serialize() : that.options.instance.context};
  ...
  }, 'input[data-lookup][data-drop] paste': 'immediate_find_or_create', 'input[data-lookup][data-drop] drop': 'immediate_find_or_create', fetch_templates: function (dfd) {
    var that = this;
    dfd = dfd ? dfd.then(function () { return that.options; }) : $.when(this.options);
    return $.when(
      can.view(this.options.content_view, dfd), can.view(this.options.header_view, dfd), can.view(this.options.button_view, dfd), can.view(this.options.custom_attributes_view, dfd)
  ...
  set_value: function (item) {
    ...
    if ($elem.is('[data-list]')) {
      var list_path = name.slice(0, name.length - 1).join('.'), cur = instance.attr(list_path)
        ;
  ...
  }, '{instance} destroyed': ' hide', ' hide': function (el, ev) {
