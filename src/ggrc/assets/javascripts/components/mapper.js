...
(function (can, $) {
  ...
  var MapperModel = can.Map({
    ...
    types: can.compute(function () {
      ...
      selector_list = _.intersection.apply(_,
  ...
  can.Component.extend({
    ...
    events: {
      ...
      deferredSave: function () {
        ...
        var data = {
          multi_map: true,
          arr: _.compact(_.map(this.scope.attr('mapper.selected'),
            function (desination) {
              var isAllowed = GGRC.Utils.allowed_to_map(source, desination);
              var inst = _.find(this.scope.attr('mapper.entries'),
                function (entry) {
      ...
      addNew: function (el, ev, model) {
        ...
        binding = selected.get_binding(get_binding_name(selected,
          model.constructor.table_plural));
        mapping = GGRC.Mappings.get_canonical_mapping_name(selected.type,
          model.type);
      ...
      '.modal-footer .btn-map click': function (el, ev) {
        ...
        que.enqueue(instance).trigger().done(function (inst) {
          ...
          _.each(this.scope.attr('mapper.selected'), function (destination) {
            ...
            mapping = GGRC.Mappings.get_canonical_mapping(object,
              isAllObject ? destination.type : type);
      ...
      setBinding: function () {
        ...
        selected = this.scope.attr('mapper.get_instance');
        table_plural = get_binding_name(selected,
    ...
    scope: {
      ...
      checkbox: can.compute(function (status) {
        return /true/gi.test(this.attr('is_mapped')) ||
          this.attr('select_state') || this.attr('appended');
      })
    },
    events: {
  ...
  can.Component.extend({
    ...
    events: {
      ...
      drawPage: function () {
        ...
        page_items = this.scope.attr('entries').slice(page * per_page,
          next_page * per_page);
        ...
        return que.enqueue(_.pluck(page_items, 'instance')).trigger()
      ...
      searchFor: function (data) {
        ...
        if (!_.includes(['ObjectPerson', 'WorkflowPerson'], join_model) &&
          ...
          data.options.__permission_type = data.options.__permission_type ||
            'update';
        }
        data.model_name = _.isString(data.model_name) ? [data.model_name] :
          data.model_name;
      ...
      getResults: function () {
        ...
        filters = _.compact(_.map(this.scope.attr('mapper.relevant'),
          function (relevant) {
            var mappings;
            var Loader;
            ...
            if (model_name === 'AllObject') {
              ...
              mappings = _.compact(_.map(GGRC.Mappings.get_mappings_for(
  ...
  $('body').on('click', '[data-toggle="unified-mapper"],' +
    ' [data-toggle="unified-search"]', function (ev) {
